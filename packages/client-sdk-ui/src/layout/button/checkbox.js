 ...
 * so a click emits a `change` view event carrying the requested state
 * (`{ checked }`) and nothing else: the workflow acts on it and the new
 * state arrives back down the data path, like any other rendering.
 */
export default class CheckboxLayout extends TemplateBasedLayout {
  ...
  _handleClick(event) {
    ...
    // the rendered state is what the user toggles, so it is read off the
    // element itself — the state published to the hub carries the mapped
    // value under `data`, not `value`
