...
misocmd.push(async () => {
  ...
  function renderFacetKey(layout, { field }) {
    if (field.startsWith('custom_attributes.')) {
      field = field.substring('custom_attributes.'.length);
    }
    // TODO: transform to human readable labels
    return layout.templates.helpers.escapeHtml(field);
  }
  function renderFacetValue(layout, { value }) {
    // TODO: transform to human readable labels
  ...
  workflow.useLayouts({
    facets: {
      templates: {
        value: renderFacetValue,
        title: renderFacetKey,
      },
    },
  });
